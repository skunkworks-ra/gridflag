...
class TestZarrStore:
    def test_init_and_append(self, store):
        store.init_spw(0, n_chan=4, n_corr=2, ref_freq=1e9, chan_freqs=np.linspace(1e9, 1.1e9, 4))
    ...
    def test_spw_attrs(self, store):
        store.init_spw(2, n_chan=8, n_corr=4, ref_freq=3e9, chan_freqs=np.ones(8) * 3e9)
    ...
    def test_multiple_spws(self, store):
        ...
        for spw in [0, 1]:
            store.init_spw(spw, n_chan=1, n_corr=1, ref_freq=1e9, chan_freqs=np.array([1e9]))
            store.append(
                spw,
                0,
                np.array([0], dtype=np.int64),
...
class TestOpenReadonly:
    def test_open_existing_store(self, store, tmp_path):
        ...
        store.init_spw(0, n_chan=1, n_corr=1, ref_freq=1e9, chan_freqs=np.array([1e9]))
        store.store_grid(0, 0, "median_grid", np.ones((5, 3), dtype=np.float32))
